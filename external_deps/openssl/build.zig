...
pub fn build(b: *std.Build) void {
    ...
    crypto.addIncludePath(upstream.path("include"));
    ...
    ssl.installHeadersDirectory(upstream.path("include_gen"), "", .{});
    b.installArtifact(crypto);
    b.installArtifact(ssl);
}

fn libcrypto(b: *std.Build, upstream: *std.Build.Dependency, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) *std.Build.Step.Compile {
    ...
    lib.addCSourceFiles(.{ .root = upstream.path("crypto/"), .files = &.{
        "cpuid.c", "ctype.c",
    }, .flags = &.{} });
    return lib;
}

fn libssl(b: *std.Build, upstream: *std.Build.Dependency, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) *std.Build.Step.Compile {
    const lib = b.addStaticLibrary(.{
        .name = "ssl",
        .target = target,
        .optimize = optimize,
    ...
    lib.addCSourceFiles(.{ .root = upstream.path("ssl/"), .files = &.{
        "pqueue.c",
